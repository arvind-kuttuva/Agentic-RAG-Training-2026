...
CREATE TABLE IF NOT EXISTS multimodal_chunks (
    ...
    chunk_type   TEXT         NOT NULL CHECK (chunk_type IN ('text', 'table', 'image_caption')),
    element_type TEXT,
...
CREATE INDEX IF NOT EXISTS idx_chunks_document_id    ON multimodal_chunks (document_id);
CREATE INDEX IF NOT EXISTS idx_chunks_chunk_type ON multimodal_chunks (chunk_type);
CREATE INDEX IF NOT EXISTS idx_chunks_source_page ON multimodal_chunks (source_page);
