...
# ---------------------------------------------------------------------------
_TEXT_CHUNK_SIZE = 250
_TEXT_CHUNK_OVERLAP = 25

def _merge_small_paragraphs(elements: list[dict],
                max_block_size: int = _TEXT_CHUNK_SIZE) -> list[dict]:
    """
    Merge adjacent short text elements into larger blocks before chunking.

    Strategy:
    - Combine consecutive "text" elements until max_block_size is reached
    - Preserve metadata from the first element in the block
    - Flush buffer when encountering non-text elements (table/image)
    """
    ...
    def flush_buffer():
        nonlocal buffer_texts, buffer_len, buffer_meta
        if buffer_texts:
            merged.append({
    ...
    for elem in elements:
        ...
        if not buffer_texts:
            buffer_meta = elem["metadata"]

        # ✅ If adding this paragraph fits → append
        if buffer_len + len(text) <= max_block_size:
...
def ingest_pdf(file_path: str) -> dict:
   ...
   Steps:
     1. Register the document in the `documents` table → get a stable document_id
   ...
   Returns:
       Dict with "status", "document_id", and "chunks_ingested" count.
   """
   ...
   # can be cleaned up by document_id if needed (ON DELETE CASCADE on FK).
   document_id = upsert_document(resolved.name, str(resolved))
   print(f"[ingestion] document_id={document_id}  file={file_path}")


   # ── Step 2: Parse the PDF ─────────────────────────────────────────────────
   # parse_document() runs the full Docling pipeline and returns a flat list.
   # Each element: {content, content_type, metadata{source_page, section, …}}
   ...
   parsed_elements = _merge_small_paragraphs(parsed_elements)
   print(f"[ingestion] After merging {len(parsed_elements)} elements")
   ...
   count = store_chunks(chunks, document_id)
   print(f"[ingestion] Stored {count} chunks → multimodal_chunks")


   return {"status": "success", "document_id": document_id, "chunks_ingested": count}
